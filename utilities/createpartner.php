...
if(
	!empty($data->OrganisationName) &&
	!empty($data->Email) 
){
	$OrganisationName = htmlspecialchars(strip_tags($data->OrganisationName));
  ...
  $AboutOrganisation = htmlspecialchars(strip_tags($data->AboutOrganisation));
  $Comment = htmlspecialchars(strip_tags($data->Comment));
	$Sdgs= htmlspecialchars(strip_tags($data->Sdgs));
		
		
	$stmt = $UtilitiesManager->readPartnerByEmail($email);
        $num = $stmt->rowCount();
                
        if($num <= 0 ){  
       
       // create the product
	if($UtilitiesManager->createPartner($OrganisationName,$Website,$email,$phone,$address,$city,$state,$country,$AboutOrganisation,$Comment,$Sdgs)){

$mailBody=<<<_MAIL_
<div style="width:100%;">
    <h3 style="margin:20px 0;text-align:left;font-size:20px;font-weight:bold;">Partner Notification</h3>
...
$mail->addAddress(SMTPVolunteermail, 'PARTNER NOTIFICATION');     // Add a recipient
$mail->isHTML(true);                                  // Set email format to HTML
$mail->Subject = "PARTNER NOTIFICATION";
$mail->Body= $mailBody;
$mail->send();
...
echo json_encode(array("message" => "Registration Successful."));
	    ...
	    http_response_code(503);
            echo json_encode(array("message" => "Partner Exists."));
	}
